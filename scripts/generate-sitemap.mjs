...
function writeBlogSeoIndex() {
  const blogJsonPath = join(CONFIG_DIR, "blog-posts.json");
  if (!existsSync(blogJsonPath)) return 0;
  const blogData = JSON.parse(readFileSync(blogJsonPath, "utf8"));
  const scheduleDates = Object.fromEntries(
    (blogData.schedule || []).map((row) => [row.slug, row.iso]),
  );
  /** @type {Record<string, { title: string, description: string, lastmod?: string }>} */
  const blogIndex = {};
  for (const post of blogData.en?.posts || []) {
    const firstPara = (post.content || []).find((block) => block.type === "p" && block.text);
    const description = (firstPara?.text || post.title || "").trim().slice(0, 155);
    blogIndex[post.slug] = {
      title: post.title,
      description,
      lastmod: scheduleDates[post.slug] || lastmod,
    };
  }
  writeFileSync(
    join(ROOT, "functions/blog-seo-index.js"),
    `/** Auto-generated by scripts/generate-sitemap.mjs — do not edit. */\nexport const BLOG_SEO_INDEX = ${JSON.stringify(blogIndex, null, 2)};\n`,
    "utf8",
  );
  return Object.keys(blogIndex).length;
}

const blogCount = writeBlogSeoIndex();
if (blogCount) console.log(`Blog SEO index → functions/blog-seo-index.js (${blogCount} posts)`);
